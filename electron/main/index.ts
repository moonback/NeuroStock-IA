...
// ─── Résolution du logo (cherche dans plusieurs emplacements) ─────────────────
function resolveLogoBase64(): string {
  const candidates = [
    path.join(__dirname, 'logo-full-transaparent.png'),
    path.join(process.env.VITE_PUBLIC!, 'logo-full-transaparent.png'),
    path.join(__dirname, '../../public/logo-full-transaparent.png'),
  ];
...
function createSplash() {
  ...
  const logoMarkup = logoDataUrl
    ? `<img class="logo-img" src="${logoDataUrl}" alt="NeuroStock" />`
